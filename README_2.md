...
Una vez copiados todos los archivos del proyecto en la carpeta generada, se procederá al lanzamiento y ejecución del código. Para ello, se lanzará un terminal de Ubuntu y se realizará un comando SSH para iniciar la comunicación con el robot. Seguidamente, se ejecutará el comando LS para ver las carpetas con las que cuenta el proyecto y a continuación, se ejecutará el comando CD para entrar en la carpeta del proyecto. Mediante otra ejecución del comando LS se analizarán todos los archivos con los que cuenta el proyecto y seguidamente, se creará mediante el comando Touch un archivo vació .py para la ejecución de nuestro código. Una vez creado el archivo, mediante el comando Nano, se entrará en el mismo y se pegará en el el código que se desea ejecutar mediante ctrl+v.
A continuación, con ctrl+x saldremos del archivo y pulsando la tecla Y se guardará el código que acabamos de pegar en el archivo. 
Finalmente, para ejecutar el código, se debe pulsar la tecla enter, ecribir el nombre del archivo .py que se generó en el paso anterior; y volver a presionar la tecla enter.
